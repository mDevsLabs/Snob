...
import { useBlockBlastSpecial, GRID_SIZE } from "@/lib/blockBlastSpecial";
...
export default function SpecialMode() {
  const { 
    ...
    destroyCell, frozenDestroyed, bombsTriggered
  } = useBlockBlastSpecial();
  const { 
            ...
            >
              {grid.map((row, r) =>
                row.map((cellObj, c) => {
                  ...
                  const isFilled = cellObj.color > 0;
                  const cellColor = isFilled ? (skin.colors[cellObj.color - 1] ?? skin.colors[0]) : "";
                  ...
                  // Styles spéciaux
                  let specialClass = "";
                  if (cellObj.bomb) specialClass = "bg-orange-500/80 border-orange-400 shadow-[inset_0_0_15px_rgba(234,88,12,0.8)] animate-pulse";
                  else if (cellObj.frozen && !cellObj.cracked) specialClass = "bg-cyan-200/50 border-cyan-300 shadow-[inset_0_0_15px_rgba(34,211,238,0.8)]";
                  else if (cellObj.cracked) specialClass = "bg-slate-400/50 border-slate-300 shadow-[inset_0_0_10px_rgba(148,163,184,0.5)] border-dashed";
                  ...
                  const cellColorClass = isPreview
                    ...
                    : (isFilled ? (specialClass || cellColor) : "bg-slate-950/45 border border-white/[0.02] shadow-[inset_0_2px_4px_rgba(0,0,0,0.7)]");
                  ...
                  return (
                    ...
                    >
                      {isFilled && !specialClass && (
                        <div className="absolute top-0.5 left-0.5 right-0.5 h-[30%] bg-white/15 rounded-t-md pointer-events-none" />
                      )}
                      
                      {cellObj.bomb && <span className="absolute inset-0 flex items-center justify-center text-xl pointer-events-none drop-shadow-md">💣</span>}
                      {cellObj.frozen && !cellObj.cracked && <span className="absolute inset-0 flex items-center justify-center text-xl pointer-events-none drop-shadow-md">❄️</span>}
                      {cellObj.cracked && <span className="absolute inset-0 flex items-center justify-center text-xl pointer-events-none drop-shadow-md">💔</span>}

                      {isFilled && skin.id === "hacker" && colorblindMode !== "symbols" && !specialClass && (
                        <span className="flex items-center justify-center w-full h-full text-[9px] font-mono opacity-60 text-green-300">01</span>
                      )}
                      {isFilled && colorblindMode === "symbols" && !specialClass && (
